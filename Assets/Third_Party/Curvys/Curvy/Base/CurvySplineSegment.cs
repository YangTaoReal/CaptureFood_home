    ...
    public class CurvySplineSegment : MonoBehaviour, IComparable, IPoolable
    {
        ...
        */
        ...
        public bool CanHaveFollowUp
        {
            get 
            {
                return IsLastVisibleControlPoint || IsFirstVisibleControlPoint;
            }
        }
        /// <summary>
        ...
        public int ControlPointIndex
        {
            get
            {
                if (mControlPointIndex == -1)
                    //OPTIM set this when adding the CurvySplineSegment to the ControlPoints list, since we can easily know it's index at that time, instead of calling IndexOf
        ...
        public CurvySpline Spline
        {
            get
            {
                if (mSpline == null && transform.parent != null)
                    //OPTIM set this when adding the CurvySplineSegment to the CurvySpline, since we can easily know it's value at that time
        ...
        public void Reset()
        {
            ...
            m_ConnectionSyncPosition = false;
            m_ConnectionSyncRotation = false;
            if (Connection)
                Disconnect();
        ...
        {
            if (target == null || CanHaveFollowUp)
            {
                FollowUp = target;
                FollowUpHeading = heading;
                //OPTIM this dirtying seems unecessary. Remove it after non regression confirmation
                SetDirty();
            }
        }
        ...
        internal void refreshOrientationStaticINTERNAL()
        {
            ...
            if (Approximation.Length > 1)
            {
                ...
                Vector3 lastUp = ApproximationUp[cacheSize] = getOrthoUp1INTERNAL();
                float oneOnCachSize = 1f / cacheSize;
                for (int i=1;i<cacheSize;i++)
                {
