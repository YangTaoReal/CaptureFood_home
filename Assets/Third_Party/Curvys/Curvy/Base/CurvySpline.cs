...
namespace FluffyUnderware.Curvy
{
    ...
    public class CurvySpline : CurvySplineBase
    {
        ...
        public const string APIVERSION = "221";
        ...
        public static int CalculateCacheSize(int density, float length, float maxPointsPerUnit)
        {
            ...
            //https://www.wolframalpha.com/input/?i=plot+(8+(-1+%2B+x)%5E2)%2F9801+for+x+from+1+to+100
        ...
        public float GetNearestPointTF(Vector3 p, out Vector3 nearest, int startSegmentIndex = 0, int stopSegmentIndex = -1)
        {
            ...
            float f;
            return GetNearestPointTF(p, out nearest, out segement, out f, startSegmentIndex, stopSegmentIndex);
        }
        ...
        /// <remarks>The speed as well as the accuracy depends on the Granularity</remarks>
        /// <returns>a TF value in the range 0..1 or -1 on error</returns>
        public float GetNearestPointTF(Vector3 p, out Vector3 nearestPoint,[CanBeNull] out CurvySplineSegment nearestSegment, out float nearestSegmentF, int startSegmentIndex = 0, int stopSegmentIndex = -1)
        {
            ...
            {
                float f = this[i].GetNearestPointF(p);
...
#region --- Static ---
                    ...
                    {
#region --- Dynamic ---
                        //OPTIM there is waaay too much unecessary computations here (to define the orientation) when the spline's orientation dynamic but few or none of the CPs uses swirling (and possible other things that takes advantage of dynamic orientation). So you might have a spline that looks like a staticly oriented one, but does waay more computations than the staticly oriented equivalent.
        ...
        public void SetDirty(CurvySplineSegment controlPoint, bool dirtyCurve=true, bool dirtyOrientation=true)
        {
            if (controlPoint)
            {
                ...
                //OPTIM using a set or a sortedSet here will reduce the contains check execution time.
                //But if you do so, you will have to refactor a little bit The Refresh method, to remove the needed of sorting the dirty cps, and also avoid modifying the dirty cps list while refreshing.
                //You can still just make mDirtyControlPoints a sorted set, and use elementAt(i) instead of the list's [i], but peformances gain would not be great, so I prefer not modifying the code for a few tenths of percent gain
