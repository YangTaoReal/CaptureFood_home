...
#region ### Constructors ###
    ...
    public class CGVMesh : CGBounds
    {
        ...
        //TODO Make code contracts work with Unity
//#if NET_4_6
...
//#endif
        ...
        public static CGVMesh Get(CGVMesh data, CGVolume source, IntRegion subset, bool addUV, bool reverseNormals)
        {
            ...
            System.Array.Copy(source.VertexNormal, start, data.Normal, 0, size);
        ...
        private void MergeUVsNormalsAndTangents(CGVMesh source, int preMergeVertexCount)
        {
            ...
            {
                if (HasUV == false)
                    Array.Resize(ref UV, preMergeVertexCount);

                Vector2[] sourceArray = source.HasUV
                    ? source.UV
                    : new Vector2[source.Count];
                copyData(ref sourceArray, ref UV, preMergeVertexCount, source.Count);
            }

            if (HasUV2 || source.HasUV2)
            {
                if (HasUV2 == false)
                    Array.Resize(ref UV2, preMergeVertexCount);

                Vector2[] sourceArray = source.HasUV2
                    ? source.UV2
                    : new Vector2[source.Count];
                copyData(ref sourceArray, ref UV2, preMergeVertexCount, source.Count);
            }

            if (HasNormals || source.HasNormals)
            {
                if (HasNormals == false)
                    Array.Resize(ref Normal, preMergeVertexCount);

                Vector3[] sourceArray = source.HasNormals
                    ? source.Normal
                    : new Vector3[source.Count];
                copyData(ref sourceArray, ref Normal, preMergeVertexCount, source.Count);
            }

            if (HasTangents || source.HasTangents)
            {
                if (HasTangents == false)
                    Array.Resize(ref Tangents, preMergeVertexCount);

                Vector4[] sourceArray = source.HasTangents
                    ? source.Tangents
                    : new Vector4[source.Count];
                copyData(ref sourceArray, ref Tangents, preMergeVertexCount, source.Count);
            }
        }
